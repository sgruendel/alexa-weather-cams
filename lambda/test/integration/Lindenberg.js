...
function verifyResponse(error, stdout) {
    ...
    expect(alexaResponses[0].content.caption, 'output speech').to.equal('Hier ist die Kamera Lindenberg Nordnordost.');
}

describe('Wetterkamera Lindenberg', () => {
    it('should find webcam', (done) => {
        const args = ask.execArgs.concat(['Alexa frage Wetterkamera nach Lindenberg']);
